...
G29_TYPE GcodeSuite::G29() {
  ...
  if (!g29_in_progress) {
    ...
    dryrun = parser.boolval('D') || TERN0(PROBE_MANUALLY, no_action);
      ...
      const float x_min = X_MIN_BED, x_max = X_MAX_BED,
                  y_min = Y_MIN_BED, y_max = Y_MAX_BED;
      // SERIAL_ECHOLNPAIR("xmin: ", x_min, " ymin: ", y_min, " xmax: ", x_max, " ymax: ", y_max);
      if (parser.seen('H')) {
        ...
        if (parser.seenval('L') || parser.seenval('F') || parser.seenval('R') || parser.seenval('B')) SERIAL_ECHOLNPAIR("[INFO] All probes set 50. Please do not change.");
        probe_position_lf.set(50, 50);
        probe_position_rb.set(150, 150);
        // probe_position_lf.set(
        ...
        //   parser.seenval('L') ? RAW_X_POSITION(parser.value_linear_units()) : 50,
        //   parser.seenval('F') ? RAW_Y_POSITION(parser.value_linear_units()) : 50
        // );
        // probe_position_rb.set(
        //   parser.seenval('R') ? RAW_X_POSITION(parser.value_linear_units()) : 150, // x_max
        //   parser.seenval('B') ? RAW_Y_POSITION(parser.value_linear_units()) : 150  // y_max
        // );
      }
  ...
  #else // !PROBE_MANUALLY
  {
    ...
    //   } // outer
      ...
      // offset 50
      float xCoords[9] = {76.070, 122.990, 165.657, 
                          74.506, 121.120, 165.267, 
                          71.605, 118.128, 162.885};
      float yCoords[9] = {46.733, 37.280, 28.901, 
                          90.525, 84.371, 78.617, 
                          135.049, 130.981, 127.102};

      // offset 90
      /*float xCoords[9] = {112.472, 121.575, 130.563,
                          112.003, 121.120, 130.132,
                          111.493, 120.621, 129.654 };
      float yCoords[9] = {76.387, 75.006, 73.645,
                          85.641, 84.371, 83.118,
                          94.888, 93.718, 92.563 };*/
      
      uint8_t pt_index = 1;
      int coordinateIndex;
      for (coordinateIndex = 0; coordinateIndex < 9 && !isnan(measured_z); coordinateIndex++, pt_index++){

        TERN_(AUTO_BED_LEVELING_LINEAR, indexIntoAB[coordinateIndex/3][coordinateIndex%3] = ++abl_probe_index); // 0...

        if (verbose_level) SERIAL_ECHOLNPAIR("Probing mesh point ", int(pt_index), "/", abl_points, ".");
        
        TERN_(HAS_DISPLAY, ui.status_printf_P(0, PSTR(S_FMT " %i/%i"), GET_TEXT(MSG_PROBING_MESH), int(pt_index), int(abl_points)));

        measured_z = faux ? 0.001f * random(0, 200) : probe.probe_at_point(xCoords[coordinateIndex], yCoords[coordinateIndex], raise_after, verbose_level);
        if (isnan(measured_z)) {
          set_bed_leveling_enabled(abl_should_enable);
          break; // Breaks out of both loops
        }

        z_values[coordinateIndex%3][coordinateIndex/3] = measured_z + zoffset;
        TERN_(EXTENSIBLE_UI, ExtUI::onMeshUpdate(meshCount, z_values[coordinateIndex%3][coordinateIndex/3]));
