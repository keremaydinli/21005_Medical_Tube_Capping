...
#include "../../core/debug_out.h"
  ...
  void bumpHoming(){
    xyz_int8_t homing_bump = HOMING_BUMP_MM;
  ...
  static void quick_home_xy() {
    ...
    feedRate_t fr_mm_s = homing_feedrate(X_AXIS);
    xyz_int8_t homing_bump_feedrate = HOMING_BUMP_DIVISOR;
    ...
    do_blocking_move_to_xy(X_BED_SIZE, 0.0, fr_mm_s);

    do {
      clearHoming(fr_mm_s);
    } while(!READ(X_MIN_PIN) || !READ(Y_MAX_PIN));
    bumpHoming();
    do {
      clearHoming(fr_mm_s, homing_bump_feedrate);
    } while(!READ(X_MIN_PIN) || !READ(Y_MAX_PIN));
...
void GcodeSuite::G28() {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    // //   homeaxis(Y_AXIS);
    // if ((doY || (ENABLED(CODEPENDENT_XY_HOMING) && doX)))
    //   homeaxis(Y_AXIS);
